...
> **说明：** 协议检测阶段不区分 `/v1/chat/completions` 和 `/v1/responses`，
> 两者都返回 `Protocol::OpenAI`。端点类型的区分（决定使用 `reasoning_effort`
> 还是 `reasoning.effort` 字段）是在思考注入阶段完成的，详见 `thinking-injector/spec.md`。
