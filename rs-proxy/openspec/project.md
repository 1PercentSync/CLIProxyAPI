...
**CLIProxyAPI 行为：** 当模型不支持动态预算时，计算中点并在 `mid <= 0` 时有额外回退逻辑

**RS-Proxy 行为：** 直接返回中点，省略 `mid <= 0` 分支

**理由：**
- 当前所有模型定义中 `min + max > 0`，`mid` 永远不会 <= 0
- RS-Proxy 要求模型必须在注册表中，可保证模型定义的合理性
- 简化实现，避免不可达代码

### 空括号处理

**CLIProxyAPI 行为：** 对空括号 `model()` 返回原始模型名（含括号）

**RS-Proxy 行为：** 对空括号 `model()` 去除括号，使用 `model` 作为基础模型名

**理由：**
- 提供更干净的模型名称
- 空括号显然是用户的无意输入或占位符，应当被清理
