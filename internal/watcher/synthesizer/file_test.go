...
func TestFileSynthesizer_Synthesize_ConfigPriorityOverridesFilePriority(t *testing.T) {
	tempDir := t.TempDir()
	...
	if got := auths[0].Attributes["priority"]; got != "99" {
		t.Fatalf("expected priority 99 (config override), got %q", got)
	}
}
