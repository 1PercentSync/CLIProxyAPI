...
func (m *Manager) executePriorityOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel string, tried map[string]struct{}) (cliproxyexecutor.Response, error) {
	...
	for _, candidate := range candidates {
		...
		if accountType == "api_key" {
			if proxyInfo != "" {
				entry.Debugf("Use API key %s for model %s %s (priority: %d)", util.HideAPIKey(accountInfo), req.Model, proxyInfo, auth.Priority)
			} else {
				entry.Debugf("Use API key %s for model %s (priority: %d)", util.HideAPIKey(accountInfo), req.Model, auth.Priority)
			}
		} else if accountType == "oauth" {
			if proxyInfo != "" {
				entry.Debugf("Use OAuth %s for model %s %s (priority: %d)", accountInfo, req.Model, proxyInfo, auth.Priority)
			} else {
				entry.Debugf("Use OAuth %s for model %s (priority: %d)", accountInfo, req.Model, auth.Priority)
			}
		}
